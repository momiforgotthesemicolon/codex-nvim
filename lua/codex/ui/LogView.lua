...
--- Open the most recent Codex log and map "q" to return to the previous
--- buffer. Stores the originating window, buffer, and cursor so the log view
--- can jump back.
function M.openLastLog()
  ...
  pcall(vim.api.nvim_buf_set_var, logBuf, "codexLogReturn", {
    win = returnWin,
    buf = returnBuf,
    cursor = returnCursor,
  })

  local message = "Press \"q\" to return to your previous view."
  vim.notify(message, vim.log.levels.INFO)
  vim.keymap.set("n", "q", function()
    local ok, data = pcall(vim.api.nvim_buf_get_var, logBuf, "codexLogReturn")
    if ok and data then
