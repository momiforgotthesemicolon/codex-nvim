...
---@param includeFullBuffer boolean|nil whether to include full buffer contents
...
function M.buildPrompt(
  ...
  local taskTitle = taskLabel
    or "Replace the full buffer with the correct output, using the provided "
      .. "scope as guidance."
  local returnLabel = "Return only the full buffer text with your changes "
    .. "applied."
  ...
  local promptLines = {
    ...
    "Keep in mind that you cannot interact with the user. If you face choice, "
      .. "please chose the most probable option",
    "Please return only the code output",
    "Make sure that outputs new lines and tabs style matches the initial "
      .. "buffer",
    "The selection block / Context near cursor is context only; do not "
  ...
  vim.list_extend(promptLines, {
    "",
    returnLabel,
    "Do not include markdown, backticks, commentary, or status lines. Do not "
