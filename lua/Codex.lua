...
function M.completeSelectionOrScope()
  ...
  if visualRange then
    ...
    local taskLabel = table.concat({
      "Complete the scope based on the context of the line under the cursor. ",
      "The scope could be a class, method, function, comment, variable, or ",
      "other logical block. Update the full buffer accordingly.",
    })
...
function M.completeFullBuffer()
  ...
  local taskLabel = table.concat({
    "Replace the full buffer with the generated output only in the context ",
    "of given neovim cursor. Context could be inside a function, lambda, ",
